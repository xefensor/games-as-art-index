function initializeGamesAsArtLibrary() {
  ...
  const githubUrls = {
    ...
    license: `${githubRepository}/blob/main/LICENSE`,
    contentLicense: `${githubRepository}/blob/main/CONTENT-LICENSE.md`,
  ...
  const sourceHost = item => {
    try { return new URL(resourceUrl(item)).hostname.replace(/^www\./, ""); }
    catch { return ""; }
  };
  ...
  const trustState = item => {
    ...
    let key = check ? "working" : "needs-rechecking";
  ...
  function resourceCard(item, recommendationReason = "") {
    ...
    return `<article class="resource-card ${saved ? "is-saved" : ""} ${visited ? "is-visited" : ""}" data-resource-id="${item.id}" data-queue-state="${status}">
      <a class="card-main" href="#/resource/${item.id}">${cover(item)}${state ? `<span class="cover-state">${state}</span>` : ""}<div class="card-copy">
        ...
        <h3>${item.title}</h3><p class="card-credit">By ${escapeHtml(item.creator)} · ${item.year}</p>
        <p class="card-source"><span>Source</span><strong>${escapeHtml(item.publisher)}</strong>${sourceHost(item) ? `<small>${escapeHtml(sourceHost(item))}</small>` : ""}</p>
  ...
  function resourceListItem(item) {
    ...
    const status = queueStatus(item.id);
    const state = saved ? queueBadgeLabels[status] : visited ? "Visited" : "";
    return `<article class="resource-list-item ${saved ? "is-saved" : ""} ${visited ? "is-visited" : ""}" data-resource-id="${item.id}" data-queue-state="${status}"><a class="list-main" href="#/resource/${item.id}"><div class="list-title"><span>${item.type}</span><strong>${item.title}</strong><small>By ${escapeHtml(item.creator)} · ${item.year}</small></div><dl><div class="list-source"><dt>Source</dt><dd title="${escapeHtml(item.publisher)}">${escapeHtml(item.publisher)}</dd><small>${escapeHtml(sourceHost(item))}</small></div><div><dt>Subject</dt><dd>${item.subjects[0]}</dd></div><div><dt>Access</dt><dd>${item.access}</dd></div><div><dt>Length</dt><dd>${item.length}</dd></div><div><dt>Level</dt><dd>${item.level}</dd></div><div><dt>Verified</dt><dd>${trustBadge(item, true)}</dd></div></dl><span class="list-state ${state ? "" : "is-empty"}" aria-hidden="${state ? "false" : "true"}">${state || "Status"}</span></a><div class="list-actions">${queueControl(item, true)}<a class="direct-control" href="${escapeHtml(resourceUrl(item))}" target="_blank" rel="noreferrer" aria-label="Open original resource: ${escapeHtml(item.title)}">Open original ↗</a>${reportButton(item)}</div></article>`;
  }
  ...
  function collectionCard(item) {
    ...
    return `<a class="collection-card" data-color="${item.color}" href="#/collection/${item.id}">
      ...
      <div><span class="eyebrow">Curated learning path</span><h3>${item.title}</h3><p>${item.description}</p></div>
  ...
  function siteFooter() {
    return `<footer class="site-footer shell"><div><a class="brand footer-brand" href="#/"><span class="brand-bars"><i></i><i></i><i></i></span><span>Games <em>as</em> Art <small>Index</small></span></a><p>A curated index of game development, history, and criticism.</p></div><nav><a href="#/browse">Browse all</a><a href="#/collections">Collections</a><a href="#/saved">Saved locally</a><a href="#/suggest">Contribute</a><a href="#/about">About the index</a></nav><small>${resources.length} records · English · Browsing needs no account · Contributions reviewed on GitHub<br><a href="${githubUrls.license}" target="_blank" rel="noreferrer">MIT code</a> · <a href="${githubUrls.contentLicense}" target="_blank" rel="noreferrer">CC BY 4.0 content</a> · Third-party media excluded</small></footer>`;
  }
  ...
  function collectionView(item) {
    ...
    return `<section class="collection-page shell" data-color="${item.color}" data-collection-id="${item.id}"><p class="breadcrumb"><a href="#/collections">Collections</a> / ${item.title}</p><header class="collection-hero"><div><span class="eyebrow">Curated learning path · ${progress.total} resources</span><h1>${item.title}</h1><p>${item.description}</p></div><aside><div class="collection-stat-grid"><div><strong>${progress.finished}/${progress.total}</strong><span>finished</span></div><div><strong>${remaining}</strong><span>remaining</span></div><div><strong>${progress.commitment}</strong><span>estimated commitment</span></div></div><div class="collection-progress large"><span><i style="width:${progress.percent}%"></i></span><small>${progress.percent}% complete</small></div><div class="collection-actions"><button type="button" data-add-collection="${item.id}" ${allQueued ? "disabled" : ""}>${allQueued ? "All resources in queue" : "Add path to queue"}</button>${progress.next ? `<a class="primary-action" href="#/resource/${progress.next.id}">${progress.finished === progress.total ? "Review from the beginning" : progress.queued ? "Continue path" : "Start path"} →</a>` : ""}</div></aside></header><div class="sequence-heading"><div><span class="eyebrow">Suggested order</span><h2>Work through the path</h2></div><p>Status controls update your local learning queue. You can follow the order or open any resource directly.</p></div><ol class="collection-sequence">${progress.items.map((entry, index) => `<li><span>${String(index + 1).padStart(2, "0")}</span>${resourceCard(entry)}</li>`).join("")}</ol></section>${siteFooter()}`;
  }
  ...
  function suggestView() {
    return `<section class="suggest-page shell"><header class="page-heading contribution-heading"><div><span class="eyebrow">Contribute through GitHub</span><h1>Choose the smallest useful contribution.</h1><p>The Index does not have a separate account or submission database. GitHub provides the public inbox, discussion, file history, review, and automated checks.</p></div><div class="page-count"><strong>3</strong><span>ways to contribute</span></div></header>
      <div class="contribution-options">
        ...
        <article><span class="contribution-number">03</span><div><span class="eyebrow">Issue optional</span><h2>Propose an exact edit</h2><p>If you already know a small correction, edit the relevant JSON source file and open a pull request directly. New resources usually begin with the suggestion form because they need wider editorial and asset work.</p></div><a href="${githubUrls.contributing}" target="_blank" rel="noreferrer">Read the pull-request guide ↗</a></article>
      </div>
  ...
  function catalogueBuilderView() {
    ...
    return `<section class="catalogue-builder"><header class="saved-section-header"><div><span class="eyebrow">Pull-request preparation tool</span><h2>Prepare catalogue records</h2></div><p>Create or revise records locally, run the same structural checks in the browser, then download JSON for a GitHub pull request. This tool does not publish changes by itself.</p></header>
      ...
      <div class="catalogue-builder-export"><div><strong id="catalogueExportSummary">${draftCount ? `${draftCount} staged` : "No staged changes"}</strong><span>One valid draft downloads as <code>content/resources/&lt;id&gt;.json</code>. For a new resource, that file is only one part of the pull request: catalogue ordering, a thumbnail record, generated catalogue data, and the editorial audit must also be updated. External link verification is maintained separately.</span></div><button type="button" id="exportUpdatedCatalogue" disabled>Download staged JSON</button></div><p class="catalogue-import-status" id="catalogueImportStatus" role="status" aria-live="polite"></p>
    </section>`;
  }
  ...
  function search(query = "") {
    ...
    searchResults.innerHTML = `<p class="search-status">${term ? `${matches.length} close ${matches.length === 1 ? "match" : "matches"} shown` : "Recommended starting points"}</p>${matches.map(item => `<a href="#/resource/${item.id}" data-search-route><span>${item.type}</span><div class="search-result-copy"><strong>${item.title}</strong><small>By ${escapeHtml(item.creator)} · Source: ${escapeHtml(item.publisher)}${sourceHost(item) ? ` · ${escapeHtml(sourceHost(item))}` : ""}</small></div><b>→</b></a>`).join("")}${noResults}${term ? `<a class="all-results" href="#/browse?q=${encodeURIComponent(query)}" data-search-route>Browse all matching resources →</a>` : ""}`;
